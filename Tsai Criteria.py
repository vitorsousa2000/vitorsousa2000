...
for k in range (len(comp3)): #loop para varrer os dados
    ...
    PSFE = 0
    CRIT = 0
    for w in range (3): #loop para varrer cada composição
        ...
        if(PSFE_el[w]==0): #aqui, caso o elemento seja A (=0), iremos somar sua % atômica em PSFE_a
            PSFE_a += comp3[k][w]
        if(PSFE_el[w]==1): #aqui, caso o elemento seja B (=0), iremos somar sua % atômica em PSFE_b
            PSFE_b += comp3[k][w]
    ...
    if(PSFE>=45):
        if(6.88<VEC<7.84):
            ...
            CRIT = 75
    a[comp3[k][0], comp3[k][1], comp3[k][2]] = CRIT #Aqui acrescentamos as composições ao dicionário juntamente com a informação
print(a)                                            #se haverá ou não fase sigma (CRIT=1 significa que há fase sigma, CRIT=0 que não)

#nas linhas seguintes iremos definir os parâmetros do diagrama
import ternary 
figure, tax = ternary.figure(scale=100)

fontsize = 15

tax.right_corner_label("V", fontsize=fontsize, zorder=3)
